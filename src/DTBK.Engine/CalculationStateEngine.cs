...
public sealed class CalculationStateEngine
{
    ...
    {
        var executionContext = EsconEngineContract.RequireContext(context);
        ArgumentNullException.ThrowIfNull(items);
        var normResult = _normCost.CalculateWithRequirements(items, provinceId, executionContext.Context.AsOfDate);
        var lines = normResult.Lines; var transport = transportByItem ?? new Dictionary<long, decimal>();
        var summary = _cost.Calculate(lines, workType, scenario, location, executionContext.Context.AsOfDate, vatCategory, transportAmount: transport.Values.Sum());
        var states = lines.Select(line => new CalculationLineState(line, line.Material, line.Labor, line.Machine, transport.TryGetValue(line.ProjectItemID, out var amount) ? amount : 0m, 0m, provenanceByItem is not null && provenanceByItem.TryGetValue(line.ProjectItemID, out var source) ? source : new CalculationProvenance(executionContext.Context.DataPackVersion, "", "", executionContext.Context.AsOfDate, null, executionContext.Context.ProvinceCode, "NormCostBridge → ProjectCostEngine", "Engine default", DTBK.Domain.VerificationStatus.NeedsVerification))).ToList();
        ...
        var inputHash = Hash(new { context = executionContext.Context, items, workType, scenario, location, provinceId, vatCategory, transport = canonicalTransport, provenance = canonicalProvenance });
        var trace = new CalculationTraceEngine().Create("CALCULATION_STATE", inputHash, Hash(summary), Version, new[] { new CalculationTraceStep("INPUT", "Context", executionContext.Context.ProjectId, executionContext.Context.DataPackVersion, executionContext.Context.DataPackVersion, "S01_CONTEXT"), new CalculationTraceStep("RESOLVER", "NormCostBridge", "ProjectItem → CostLine", executionContext.Context.DataPackVersion, executionContext.Context.DataPackVersion, "S03_RESOLVE"), new CalculationTraceStep("ENGINE", "ProjectCostEngine", "CostLine → ProjectCostSummary", summary.LegalVersion, summary.LegalVersion, "S10_S14_CALCULATE"), new CalculationTraceStep("RESULT", "CalculationState", states.Count.ToString(), executionContext.Context.DataPackVersion, inputHash, "STATE_CREATE"), new CalculationTraceStep("PROJECTION", "T01-T08", "Available from CalculationState", "", inputHash, "STATE_PROJECT") });
        return Capture(executionContext.Context, states, summary, trace, normResult.Requirements);
    }
