...
{
    // Mirrors the observed ESCON Cell.cs SPVALUE boundary:
    // regex -> key -> Contains -> get_Item -> Object.ToString -> Replace.
...
// Quoted compatibility form is intentionally accepted by DTBK.
...
// Missing key must not fall back to a provider.
var missingRuntime = new EsconFormulaRuntime(provider);
try
{
...
{
    if (provider.SpecialResolveCalls != 0)
...
sealed class NullProvider : IEsconValueProvider
{
